<template>
  <div class="page-container">
    <div class="page-header">
      <h2>分类管理</h2>
      <button @click="openCreateModal" class="btn-primary">+ 新增分类</button>
    </div>
...
const load = async () => {
  try {
    const response = await fetch('/api/cms/category/list?page=1&pageSize=100')
...
const save = async () => {
  try {
    const url = editingId.value ? `/api/cms/category/${editingId.value}` : '/api/cms/category'
...
const delete_ = async (id) => {
  ...
  try {
    const response = await fetch(`/api/cms/category/${id}`, { method: 'DELETE' })
    const data = await response.json()
    if (data.code === 0) {
      alert('删除成功')
      load()
...
.btn-close { background: none; border: none; font-size: 24px; color: #999; cursor: pointer; padding: 0; }
